...
var rootCommand = new RootCommand(
    "File Verifier - Verifies that a sorted file is properly ordered and matches original line count."
);

var sortedFileOption = new Option<string>(name: "--sorted-file", description: "The path to the sorted file to verify.")
{
    IsRequired = true,
};
...
rootCommand.SetHandler(
    async (sortedFilePath, originalFilePath, encodingName, verbose) =>
    {
        ...
        try
        {
            ...
            logger.LogInformation(
                "Sorted file lines processed: {SortedFileLinesProcessed}",
                result.SortedFileLinesProcessed
            );
            logger.LogInformation(
                "Original file lines processed: {OriginalFileLinesProcessed}",
                result.OriginalFileLinesProcessed
            );
            ...
            {
                logger.LogInformation(
                    "ERROR: Line count mismatch. Original: {OriginalFileLinesProcessed}, Sorted: {SortedFileLinesProcessed}",
                    result.OriginalFileLinesProcessed,
                    result.SortedFileLinesProcessed
                );
                logger.LogError(
                    "Line count mismatch. Original: {OriginalFileLinesProcessed}, Sorted: {SortedFileLinesProcessed}",
                    result.OriginalFileLinesProcessed,
                    result.SortedFileLinesProcessed
                );
            }
            ...
            logger.LogInformation(
                "File is properly sorted: {IsProperlySorted}",
                result.IsProperlySorted ? "YES" : "NO"
            );
            logger.LogInformation(
                "Verification time: {VerificationTimeTotalSeconds} seconds",
                result.VerificationTime.TotalSeconds
            );

            if (result.FoundDuplicateStringPart)
            {
                logger.LogInformation(
                    "Found at least one instance of duplicate string parts in consecutive lines: YES"
                );
            }
            else
            {
            ...
            if (!result.IsProperlySorted)
            {
                ...
                logger.LogInformation(
                    "Violation at sorted file line: {ViolationLineNumber}",
                    result.ViolationLineNumber
                );
                logger.LogInformation(
                    "Previous line ({ViolationLineNumberPrevious}): {PreviousLineContent}",
                    result.ViolationLineNumber - 1,
                    result.PreviousLineContent
                );
                logger.LogInformation(
                    "Current line  ({ViolationLineNumberCurrent}): {CurrentLineContent}",
                    result.ViolationLineNumber,
                    result.CurrentLineContent
                );
                logger.LogError(
                    "Sorting violation at sorted file line: {ViolationLineNumber}. Previous: '{PreviousLineContent}', Current: '{CurrentLineContent}'.",
                    result.ViolationLineNumber,
                    result.PreviousLineContent,
                    result.CurrentLineContent
                );
            ...
            {
                logger.LogInformation(
                    "File verification completed successfully - file is properly sorted, line counts match, and duplicate string part check passed!"
                );
                Environment.Exit(0);
            }
        }
