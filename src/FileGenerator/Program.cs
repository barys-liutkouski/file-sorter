...
var fileSizeOption = new Option<long>(name: "--size", description: "The size of the file to generate in megabytes.")
{
    IsRequired = true,
};

var outputFileOption = new Option<string>(name: "--output", description: "The output file path.")
{
    IsRequired = false,
};
...
rootCommand.SetHandler(
    async (size, outputPath, stringReuseChance) =>
    {
        ...
        logger.LogInformation(
            "Generating a file of size: {FileSize:N0} megabytes with string reuse chance: {StringReuseChance}",
            size,
            stringReuseChance
        );
